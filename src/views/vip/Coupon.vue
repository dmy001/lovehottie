<template>
  <div class="bg-white coupon">
    ...
    <div v-show="isShowList" class="mt-36 pb-72">
      ...
      <p class="mt-4 mb-2 ml-4 text-xs text-black">{{$trans('没有优惠券哦~')}}</p>
      <p class="mb-10 ml-4 text-xs text-black">{{$trans('去充值获取优惠券吧~')}}</p>
      <router-link to="/home/vip/RechargeGold">
        <p class="recharge pt-1.5 border rounded-3xl text-white">{{$trans('去充值')}}</p>
      </router-link>
    </div>
    <div v-show="!isShowList" class="text-left text-gray-800 mt-16 pb-16">
      <div>
        <p class="text-gray-700 font-bold">{{$trans('优惠券基本使用说明：')}}</p>
        ...
        <div class="flex flex-row">
          ...
          <p>{{$trans('商品的价值低于优惠券的价值，则直接使用优惠券而不找零')}}</p>
        </div>
        <div class="mt-3 mb-3 flex flex-row">
          <p class="w-5 h-5 bg-gray-200 rounded-full pl-1.5 mr-1">5</p>
          <p>{{$trans('优惠券获取方式通过LoveHottie的买赠，活动参与，充值等形式获取')}}</p>
        </div>
        <div class="flex flex-row">
          <p class="w-5 h-5 bg-gray-200 rounded-full pl-1.5 mr-1">6</p>
          <p>{{$trans('优惠券过期作废，即不能够在购买LoveHottie内商品使用')}}</p>
        </div>
      </div>
      <div class="mt-8">
        ...
        <p>② 价值为3金币的高级会员优惠券可以购买任意高级会员使用；</p>
